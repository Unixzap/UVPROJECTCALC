(async function(){
  try{
    ...
    const script=document.createElement('script');script.src='app.js?build=1049';script.onload=()=>window.dispatchEvent(new Event('uvpc-storage-ready'));document.body.appendChild(script);
  }catch(error){
    console.error('Local database startup failed',error);
    window.__UVPC_INITIAL_STATE__=null;
    const script=document.createElement('script');script.src='app.js?build=1049';document.body.appendChild(script);
  }
